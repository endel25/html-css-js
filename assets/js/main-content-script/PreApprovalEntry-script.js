...
async function fetchVisitorData(page = 1, limit = 10, retries = 3) {
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            const url = `https://192.168.3.74:3001/appointment?page=${page}&limit=${limit}`;
...
async function fetchVisitorById(visitorId, retries = 3) {
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            const response = await fetch(`https://192.168.3.74:3001/appointment/${visitorId}`, {
                method: 'GET',
                headers: { 'Content-Type': 'application/json' },
            });
            if (!response.ok) {
...
async function openVisitorModal(visitorId) {
    ...
    const setPhotoPreview = async (previewId, path, visitorId, type) => {
        ...
        if (path && path.trim() !== '') {
            try {
                const photoUrl = `https://192.168.3.74:3001/appointment/${visitorId}/photo?type=${type}`;
...
document.getElementById('approveGatepass').addEventListener('click', async () => {
    ...
    async function attemptApprove(attempt = 1, maxAttempts = 3) {
        try {
            const response = await fetch(`https://192.168.3.74:3001/appointment/${visitorId}/gatepass`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
