...
async function fetchPurposeOfVisits() {
    try {
        const response = await fetch('https://192.168.3.75:3001/purpose-of-visit');
        if (!response.ok) {
...
async function fetchTimeUnits() {
    try {
        const response = await fetch('https://192.168.3.75:3001/time-duration-unit');
        if (!response.ok) {
            throw new Error(`HTTP error! Status: ${response.status} - ${response.statusText}`);
        }
...
async function fetchVisitorTypes() {
    try {
        const response = await fetch('https://192.168.3.75:3001/visitor-type');
        if (!response.ok) {
            throw new Error(`HTTP error! Status: ${response.status} - ${response.statusText}`);
        }
...
async function checkFormStatus(email, date, time) {
    try {
        const response = await fetch(
            `https://192.168.3.75:3001/appointment/check-status?email=${encodeURIComponent(email)}&date=${encodeURIComponent(date)}&time=${encodeURIComponent(time)}`,
            {
                method: 'GET',
                headers: { 'Content-Type': 'application/json' }
            }
