        ...
        async function checkFormStatus(email, date, time) {
            try {
                const response = await fetch(
                    `http://192.168.106.137:3001/appointment/check-status?email=${encodeURIComponent(email)}&date=${encodeURIComponent(date)}&time=${encodeURIComponent(time)}`,
                    {
                        method: 'GET',
        ...
        async function initializeForm() {
            ...
            form.addEventListener('submit', async e => {
                ...
                try {
                    const response = await fetch('http://192.168.106.137:3001/appointment/create', {
                        method: 'POST',
                        body: formattedData
                    });
