...
async function fetchPurposeOfVisits() {
    try {
        const response = await fetch('http://192.168.3.74:3001/purpose-of-visit');
        if (!response.ok) {
...
async function fetchTimeUnits() {
    try {
        const response = await fetch('http://192.168.3.74:3001/time-duration-unit');
        if (!response.ok) {
            throw new Error(`HTTP error! Status: ${response.status} - ${response.statusText}`);
        }
...
async function fetchVisitorTypes() {
    try {
        const response = await fetch('http://192.168.3.74:3001/visitor-type');
        if (!response.ok) {
            throw new Error(`HTTP error! Status: ${response.status} - ${response.statusText}`);
        }
