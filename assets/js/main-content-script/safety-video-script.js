      ...
      async function checkFormStatus(email, date, time) {
        try {
          const response = await fetch(
            `http://192.168.3.74:3001/appointment/safety-status?email=${encodeURIComponent(email)}&date=${encodeURIComponent(date)}&time=${encodeURIComponent(time)}`,
            {
              method: 'GET',
      ...
      async function initializeForm() {
        ...
        form.addEventListener('submit', async (e) => {
          ...
          try {
            ...
            const response = await fetch('http://192.168.3.74:3001/appointment/safety', {
              method: 'PATCH',
              headers: { 'Content-Type': 'application/json' },
