    ...
    async function checkFormStatus(email, date, time) {
        try {
            const response = await fetch(
                `https://192.168.3.74:3001/appointment/nda-status?email=${encodeURIComponent(email)}&date=${encodeURIComponent(date)}&time=${encodeURIComponent(time)}`,
                {
                    method: 'GET',
    ...
    async function initializeForm() {
        ...
        form.addEventListener('submit', async (e) => {
          ...
          `;
            ...
            try {
                const response = await fetch('https://192.168.3.74:3001/appointment/nda', {
                    method: 'PATCH',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email,
